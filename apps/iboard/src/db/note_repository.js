class NoteRepository {
  ...
  create(note) {
    return this.dao.run('INSERT INTO notes (content,createDate,createTime) VALUES (?,?,?)', [
      note.content,
      note.createDate,
      note.createTime,
    ]);
  ...
  update(note) {
    ...
    return this.dao.run(`UPDATE notes SET content = ? WHERE id = ?`, [content, id]);
  }
  ...
  getByPage(page, size = 10) {
    ...
    return this.dao.all(`SELECT * FROM notes order by id desc limit ${size} offset ${offset};`);
  }
  ...
  getByDate(date) {
    return this.dao.all(`SELECT * FROM notes WHERE createDate = ? order by id desc`, [date]);
  }
