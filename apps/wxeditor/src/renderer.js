...
var editor = pell.init({
	...
	actions: ['bold', 'italic','underline', 'strikethrough', 'heading1', 'heading2', 'paragraph', 'olist', 'ulist', 'code', 'line'],
			...
			download(filelink,filename,function(dfile){
				upload(filename,function(res){
					...
					if(current === total){
						...
						dialog.showMessageBox({ message: '共发现'+total+'张图片，处理成功'+current+'张。' })
					}
				})
			})
		})
